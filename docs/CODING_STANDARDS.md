...
- **12.1c Analytics consent is tri-state, and absent means off.** `ConfigStore.analyticsOptIn` is `Bool?`: `nil` = never asked, `false` = declined, `true` = allowed. Read it through `isAnalyticsAllowed`, never `?? true` at a call site — collapsing `nil` and `false` re-prompts every user who declined, on every launch. Two ordering rules go with it: `FIREBASE_ANALYTICS_COLLECTION_ENABLED` stays `NO` in `Info.plist` (Firebase otherwise collects from `configure()`, i.e. before consent), and opting out must apply **within the running session** via `setCollectionEnabled(false)`, not at next launch. The consent ask must never gate the mandatory legal acceptance — privacy policy §6.3 promised opt-in, so a pre-ticked box or a disabled Continue breaks a published commitment.
