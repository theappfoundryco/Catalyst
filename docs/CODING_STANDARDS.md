...
- **12.1 Telemetry facade.** `Telemetry/Telemetry.swift` is the ONLY place a provider may be wired in; everywhere else calls `Telemetry.log(_:)` or `.set(_:)`. The payload is two events and one property — `app_open`, `feature_opened` (screen title only), `brew_installed` — and widening it is a **privacy-policy change requiring a version bump and re-consent**, not an implementation detail. New events go in `AppEvent`, new segmentation in `AppUserProperty`, derivation in `TelemetryProfile`. **Never log anything user-identifying** — no file paths, package names, hostnames, email, or device identifiers. `AppEvent` carries a screen title from a fixed 25-case enum, never a free-form string, so no call site *can* widen it. `.setUser` is deprecated and empty: it used to pass `IOPlatformUUID`, which is personal data under GDPR and which privacy policy §4 promises Catalyst does not collect. `.nonFatal`/`.breadcrumb`/`.setKey` are empty too — no crash reporter is linked, and v1.4 added usage analytics ONLY.
- **12.1b Telemetry must never be able to break launch.** The previous provider (Firebase) called `FirebaseApp.configure()` from `start()`, which hard-crashes when its config plist is absent — so *deleting a config file* would have killed the app on open rather than quietly disabling analytics. Anything optional that runs at startup must fail as a no-op. `start()` resolves `FirebaseOptions(contentsOfFile:)` and returns quietly when the gitignored plist is absent — which is the normal state of any build from the public repo, so a source build sends nothing by construction. Test by deleting the plist and launching.
- **12.1c Analytics consent is tri-state, and absent means off.** `ConfigStore.analyticsOptIn` is `Bool?`: `nil` = never asked, `false` = declined, `true` = allowed. Read it through `isAnalyticsAllowed`, never `?? true` at a call site — collapsing `nil` and `false` re-prompts every user who declined, on every launch. Two ordering rules go with it: `FIREBASE_ANALYTICS_COLLECTION_ENABLED` stays `NO` in `Info.plist` (Firebase otherwise collects from `configure()`, i.e. before consent), and opting out must apply **within the running session** via `setCollectionEnabled(false)`, not at next launch. **"On by default" is the gate checkbox being pre-ticked, NOT `nil` meaning yes.** `isAnalyticsAllowed` still requires an explicit `true`, so nothing is collected between launch and the moment the user sees the choice, and a decode failure still fails closed — never simplify it to `optIn ?? true`. The consent ask must never gate the mandatory legal acceptance: Continue is disabled on the legal checkbox only, never on the analytics one. Policy 1.3 promised opt-in and 1.4 retracts that in writing; if the default is ever flipped back, the policy text moves with it in the same release.
...
- **12.54 If a command forks helper processes, redirect its output to a file — never read it through a `Pipe` (2026-07-30).** Cost a hang with no timeout able to rescue it. Reading `system_profiler -json SPPowerDataType` over `Pipe()` wedged the battery view on "Reading battery telemetry…" permanently: `system_profiler` forks helper reporters that **inherit the write end of the pipe**, so when `system_profiler` itself exits, `executeProcess`'s `terminationHandler` runs and its `readToEnd()` blocks forever waiting for an EOF that cannot arrive while a surviving grandchild holds the descriptor. `timeoutSeconds` is no defence — it kills only the process the runner launched, and by then that process has already exited cleanly, so the timeout task is cancelled before the block even starts. **Rule:** for any command known or suspected to spawn helpers (`system_profiler`, `smartctl`, anything driving a system daemon), write to a temp file inside the shell command (`… > 'path' 2>/dev/null`) and read the file afterwards — the pipes then carry nothing, the shell exits, and `readToEnd()` returns immediately. Keep the timeout as well; it still guards the shell itself. This is the same family as 12.17: the failure is always a blocking read that no longer has anyone to unblock it. **Update (2026-08-08):** `AsyncProcessRunner`'s `terminationHandler` no longer calls `readToEnd()` at all — it drains a non-blocking descriptor via `drainAvailable(_:)` and stops at `EAGAIN`, so a surviving grandchild can no longer wedge the runner for *any* command. That closes the hole for callers who don't know their command forks; the rule above still stands as the better option, because redirecting to a file avoids the buffering entirely and keeps output intact when a fast writer outruns the readability handler.
- **12.55 A toolbar item must keep ONE stable root view — swap the label, never the item (2026-07-30).** The macOS 26 Liquid Glass capsule collapsed around the spinner every time a refresh ran. The pattern was `if busy { ProgressView() } else { Button { … } }` **at the root of the `ToolbarItem`** — which changes the item's identity, so SwiftUI tears the Button down and the glass background re-measures against the spinner's much smaller intrinsic width. It was hand-rolled in ten views. **Rule:** render one Button unconditionally and swap only its *label*, keeping the outgoing label in the layout at `.opacity(0)` (not `.hidden()`, not an inner `if` — both drop it from layout and reintroduce the collapse) with the spinner in an `.overlay`. Block re-entrancy with `.disabled(busy)` rather than by not rendering the Button, and set an `.accessibilityLabel` because the visible label is now invisible rather than absent. `Helpers/RefreshToolbarContent.swift` is the shared implementation — use it unless the action is synchronous or the item lives in a `ToolbarItemGroup`. **Two corollaries found on macOS 26 (2026-08-08).** (a) *Grouping*: a `ToolbarItemGroup`'s children are separate toolbar items, so Liquid Glass decides their capsule boundaries itself — Git Graph's three view controls were split three ways with the last one sharing a capsule with Refresh. One `ToolbarItem` wrapping an `HStack` is one capsule **by construction**; use that when the grouping is meant to be deliberate, and `ToolbarSpacer(.fixed,placement:)` (macOS 26+, availability-gated) to break groups apart. (b) *The rule is not only about refresh buttons*: ANY toolbar content that swaps between states must hold its footprint. Git Graph replaced three buttons with a bare `ProgressView` while loading, so the capsule shrank to ~20pt and snapped back on every repository open. Same fix — keep the resting content at `.opacity(0)` with `.disabled(true)` and overlay the spinner. Generalises beyond toolbars: any container that sizes itself to its content will re-measure when you swap the content's identity, so a busy state should always be a *modification* of the resting view, not a replacement for it. Related: a busy flag must be cleared in a `defer`, or a cancelled task leaves the control disabled forever.
- **12.57 A destructive action needs a prompt that names what it touches, and the prompt must not contradict the code (2026-08-08).** Every destructive flow in the app asked first — uninstall, venv deletion, login items, Cruft Sweeper — except **Snapshot restore**, the only one that *overwrites* rather than adds, and the one that had already destroyed a `~/.zshrc` along with npm and the CA bundle (issue #13). Its four guards were real and entirely invisible to the user. **Rules:** (a) any action that overwrites or removes user data confirms first; (b) the prompt names the actual targets — `SnapshotViewModel.pendingKinds` exists so the copy can list real sections instead of asking a generic "are you sure?", and a prompt that can't say *what* it is about to touch trains people to dismiss it; (c) escalate wording only when warranted — an additive restore shouldn't inherit the tone of one rewriting a shell profile, or the escalation stops meaning anything; (d) **the copy must match the code**: Cruft Sweeper's dialog said "Permanently Delete" over a body saying "moved to Trash", for an operation that calls `trashItem`. State the guards you do have — that is what turns an invisible safety net into a reason to trust the app.
- **12.56 A busy flag is cleared in a `defer`, always — and if something guards on it, that's not a style point, it's a deadlock (2026-07-30).** Three separate hangs in one session traced to the same four lines: `isBusy = true` … work … `isBusy = false` on the last line. The clear is skipped whenever the function doesn't reach that line — an unbounded child process wedging, a superseded scan being waited out (12.18b), or, most commonly and least obviously, **the task being cancelled because the user navigated away mid-operation**. SwiftUI cancels the `Task` behind a `.task` modifier or a button action when the view goes away, so this fires during ordinary use, not just under failure. On its own that leaves a spinner running. Paired with a `guard !isBusy else { return }` at the top of the same function it is far worse: every later call returns instantly at the guard, any aggregate `isBusy` keeps the whole screen disabled, and **nothing but relaunching the app recovers it** — `DashboardViewModel.runDetection` deadlocked exactly this way, taking `fullRefresh()` and the Refresh button with it. **Rules:** (a) clear the flag with `defer`, never a trailing assignment; (b) put the `defer` AFTER the guard, or an early return will clear the flag belonging to the operation already in flight; (c) if a flag both guards re-entry and gates UI, treat a missing `defer` as a release blocker, not a nit. `SnapshotViewModel` is the reference implementation — every `guard !isWorking` is paired with `defer { endWork() }`. Applies to `@Published` optionals used the same way (`upgradingPipFor`, `repairingPipFor`), not just `Bool`s.