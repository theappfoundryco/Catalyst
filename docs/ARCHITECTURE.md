...
the app links no crash-reporting SDK, and its analytics are on by default but collect nothing before the consent gate is shown (§49.6).
...
## 49.6 Telemetry — the facade (opt-in analytics since v1.4)
`Telemetry/Telemetry.swift` is the single choke point where the analytics provider is wired in, and the only file that links the SDK. **The payload is two events and one user property:** `app_open`, `feature_opened` (one parameter — a screen title from a fixed 25-case enum), and `brew_installed`. No crash reporter: Crashlytics was removed at v1.0 and did **not** return, so `nonFatal`/`breadcrumb`/`setKey` stay deliberately empty. v1.4 added usage analytics only.

**On by default, but absent still means off.** The gate's checkbox is pre-ticked; `nil` is never read as consent, so nothing is collected between launch and the moment the user sees the choice, and a decode failure fails closed. `Telemetry.isEnabled` reads `ConfigStore.isAnalyticsAllowed`, backed by a tri-state `Bool?` — `nil` = never asked, `false` = declined — because collapsing those two to a `Bool` makes a decline indistinguishable from a fresh install and re-prompts everyone who said no on every launch. Absent ⇒ off, so a decode failure or a hand-edited config fails closed. The ask lives on the consent gate (unticked, and **not** a precondition for the mandatory legal acceptance); the switch lives on the About screen and takes effect within the running session via `setCollectionEnabled`, not at next launch. `AppViewModel.logSessionStart()` replays `app_open` + the launch screen when the gate clears, because until that moment telemetry was disabled and a new user's entire first session would otherwise be the one session that never counts.

**Event-capture correctness (fixed with the v1.4 work).** `currentScreen`'s `didSet` does **not** fire for the value assigned in the property declaration, so `.dashboard` — the screen every session opens on — was never counted at all, only *returns* to it were. It is now reported explicitly by `logSessionStart()`. The `didSet` also carries an `oldValue != newValue` guard: `ContentView` binds `currentScreen` to `List(selection:)`, which writes on every row activation including a tap on the already-selected row, so an unguarded `didSet` inflated whichever screen a user idled on.

**Truth model.** `.gitignore` excludes `GoogleService-Info.plist` because the repo is public and that is the fixed filename Firebase tooling emits; one `git add -A` would publish the project configuration permanently. The real file is CODEOWNERS-held and distributed out of band, never committed, no placeholder tracked. `/Telemetry/` is a CODEOWNERS-protected path. **A build from the public source therefore sends nothing** — `start()` resolves the plist by path and returns quietly when absent, which is the intended open-source behaviour rather than a degraded one. **The plist is NOT a Copy Bundle Resources entry and must never become one:** `project.pbxproj` is tracked, so a resource reference ships a pointer to a gitignored file and every fresh clone fails with `Build input file cannot be found` (verified, not theorised). A Run Script phase can't do it either — `ENABLE_USER_SCRIPT_SANDBOXING = YES` denies undeclared reads, and declaring the plist restores the missing-file error. It is injected by `Scripts/cut_release.sh` after `exportArchive`, followed by a `--force` (never `--deep`) re-sign that reasserts hardened runtime + entitlements. Net effect: analytics ship only in official signed releases; dev builds send nothing.

**Gotchas.** (1) The no-argument `FirebaseApp.configure()` **hard-crashes when the plist is absent** — shipping that form would turn "cloned the repo" into "app crashes on open". Resolve `FirebaseOptions(contentsOfFile:)` first and early-return; test by deleting the plist and launching. (2) `FIREBASE_ANALYTICS_COLLECTION_ENABLED` must stay `NO` in `Info.plist` — otherwise Firebase starts collecting at `configure()`, i.e. before consent, and an opt-in that begins by collecting is not one. `start()` is the only thing that turns collection on. (3) The SDK is behind `#if canImport(FirebaseCore) && canImport(FirebaseAnalytics)` so the tree compiles with or without packages resolved. (4) Changing what is collected changes what `README.md` and the published privacy policy §6 promise; both must move in the same PR, with a `LegalConfig.bundledPrivacyVersion` bump so existing users re-consent. Full detail in `Telemetry/README.md`.

## 49.6b Destructive-action prompts (2026-08-08)
Every destructive flow confirms before acting — uninstall, venv deletion, login items, Cruft Sweeper — and as of v1.4 so does **Snapshot restore**, which was the only one that didn't and the only one that *overwrites* rather than adds. `SnapshotViewModel.pendingKinds` / `pendingTouchesShell` compute what a run would actually write, so `SnapshotView`'s dialog names real sections instead of asking a generic "are you sure?", and escalates its title and button role **only** when the shell profile is in the pending set (an additive restore of formulae and pip packages shouldn't inherit that tone, or the escalation stops carrying information). The copy states the four guards from issue #13 — refuse a smaller profile, refuse to drop `brew shellenv`, verify the backup's byte count, roll back on a failed write or `zsh -n` failure — because they were all real and all invisible, and a guard the user never learns about buys no trust. Cruft Sweeper's dialog was corrected in the same pass: it said "Permanently Delete" over a body saying "moved to Trash", for an operation that calls `trashItem`. See CODING_STANDARDS 12.57.
