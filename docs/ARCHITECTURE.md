...
>
> **Companion docs:** enforced coding ground rules → `Formrules.md` ·
...
> system taught something durable, the lesson lives in `Formrules.md` Part 12.
>
...
- **Release flow:** `./Scripts/cut_release.sh` — preflight (asserts Release config, hardened
  ...
  `MARKETING_VERSION`, and add its block to `about.json` (Formrules 12.36).
...
(Formrules 2.10). `AsyncProcessRunner` logs no command strings;
...
- **The pre-v1.0 history is not in this repo.** It lives in the original private repository. This repo starts at v1.0 with a fresh initial commit, so `git log` will not explain why a deleted system worked the way it did — Formrules Part 12 is where those lessons were preserved.
