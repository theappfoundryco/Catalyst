...
**Truth model.** `.gitignore` excludes `GoogleService-Info.plist`. Read that as a standing guard, **not** as evidence a provider is active — the file does not exist in a checkout, nothing reads it, and no SDK is linked to read it with. `GoogleService-Info` is the fixed filename Firebase tooling emits, and this repo is public, so the rule exists to make an accidental `git add -A` incapable of publishing a provider's configuration permanently. A real file, if one ever exists, is CODEOWNERS-held and distributed out of band — never committed, and no placeholder stand-in is tracked either. `/Telemetry/` is a CODEOWNERS-protected path.

**Gotchas.** (1) The removed Firebase provider called `FirebaseApp.configure()` from `start()`, which **hard-crashes when the plist is absent** — deleting the config would have killed the app on open rather than disabling analytics. Any replacement must fail as a no-op; test by deleting the plist and launching. (2) Wiring up a provider changes what `README.md` promises users (it states plainly that Catalyst sends nothing and carries an `analytics-none` badge). Both must change in the same PR. Shipping analytics while the README still claims none is the exact outcome this arrangement exists to prevent. Full detail in `Telemetry/README.md`.
