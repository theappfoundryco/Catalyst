...
enum NetworkConfig {
    ...
    /// A namespace containing static endpoint strings for the Catalyst backend.
    enum APIEndpoint {
        /// The base URL for the backend domain.
        static let baseURL = "https://catalyst-3aj.pages.dev"
        ...
        /// Lightweight liveness endpoint — the auth Worker's `/health` (tiny, purpose-built,
        /// always present). Replaces the old `python_versions.json` ping (that file was removed
        /// 2026-07; installable Python versions now come live from `brew`).
        static let healthURL = "https://catalyst-api.shivanggulati817.workers.dev/health"
