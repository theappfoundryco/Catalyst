...
final class PythonService {
    ...
    ///     scan was still running, so the next caller started a SECOND concurrent scan. At
    ///     launch, entitlement arrives ~1s in and invalidates the cache mid-scan, so gen 0 and
    ///     gen 1 overlapped and every interpreter got probed twice — the exact subprocess
