...
//      on exit code (Formrules 2.4), one failure never aborts the batch.
//
...
struct SnapshotRestoreService {
    ...
    /// a real migration hits (see aheadFeatures.md, v1.13):
                ...
                if code == 0 {
                    ...
                    if !p.requirements.isEmpty {
                        ...
                        if (try? reqText.write(to: reqURL, atomically: true, encoding: .utf8)) != nil {
                            // A venv is never externally managed → no PEP 668 flag (Formrules 2.7).
