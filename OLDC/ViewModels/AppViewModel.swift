...
final class AppViewModel: ObservableObject {
    ...
    /// Mirrors `authViewModel.state.isEntitled` so `ContentView` (which observes this VM)
    /// can hide the window toolbar while the auth gate covers the app.
    ...
    /// Guards the first full detection so it runs exactly once — when the auth gate opens
    /// (entitlement becomes `.entitled`), never during the unauthenticated sign-in window.
    ...
    init() {
        ...
        // Mirror entitlement so the toolbar/gate track it.
        self.authViewModel.$state
            .map { $0.isEntitled }
            .removeDuplicates()
            .assign(to: &$isEntitled)
        ...
        // Run the first full detection exactly once, the moment the gate opens (auth resolves
        // to entitled). Detection must NOT run during the unauthenticated sign-in window: it
        // floods `AsyncProcessRunner` with a burst of shell probes while the dashboard isn't even
        // shown, and the results are discarded — the old cause of the "first login after install"
        // hang (needing ⌘Q to recover) and the empty post-login dashboard. Gating it here also
        // avoids the double-load (launch stampede + the swap-in views re-detecting) that starved
        // the cooperative thread pool and left views like Git Graph spinning forever.
        self.authViewModel.$state
            .map { $0.isEntitled }
            .removeDuplicates()
            .filter { $0 }
            .sink { [weak self] _ in
                guard let self, !self.didRunInitialDetection else { return }
                self.didRunInitialDetection = true
    ...
    func startupChecks() async {
        ...
        // Resolve auth/entitlement in the background — the gate (ContentView) locks the
        // app until this returns `.entitled`. Detection is NOT started here: it's kicked off
        // exactly once when entitlement resolves to `.entitled` (see the `$state` sink in
        // `init`), so the shell-probe burst never runs behind the sign-in gate. Every detection
        // result is `@Published`, so the dashboard fills in as each check finishes.
        Task { await authViewModel.bootstrap() }

        // Resolve legal consent in parallel (behind the gate): refresh remote versions if the
        // 14-day window elapsed, then compute whether the blocking sheet is needed. Runs regardless
        // of auth so the requirement is ready the moment entitlement resolves.
