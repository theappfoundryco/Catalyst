...
//     relaunch), and
//   • a blocking, non-dismissable sheet that gates the entitled app until the user accepts.
//
// The sign-in checkbox (AuthGateView) records acceptance for brand-new sign-ins via
// `recordSignInConsent()`, so a new user is never double-prompted. Everyone else (existing
// installs with nothing stored yet, new-Mac logins, or a later version bump) is caught by the
// blocking sheet. Version comparison is exact-match ("accepted != current" ⇒ must re-accept), so
// any change on either axis re-prompts only for the doc(s) that changed.
//
...
final class LegalConsentViewModel: ObservableObject {
    ...
    /// User completed the sign-in consent checkbox — same as accepting current, so the blocking
    /// sheet won't immediately re-prompt a brand-new user.
    func recordSignInConsent() { acceptCurrent() }
