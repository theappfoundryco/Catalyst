...
final class UpdaterController: NSObject, ObservableObject,
    ...
    /// Release notes HTML for the pending update (from the appcast <description>), for display.
    @Published private(set) var releaseNotesHTML: String?
    ...
    func updater(_ updater: SPUUpdater, didFindValidUpdate item: SUAppcastItem) {
        let v = item.displayVersionString, notes = item.itemDescription
        DispatchQueue.main.async {
            self.releaseNotesHTML = notes
    ...
    func standardUserDriverWillHandleShowingUpdate(
        ...
        let v = update.displayVersionString, notes = update.itemDescription
        DispatchQueue.main.async {
...
struct CatalystApp: App {
    ...
    init() { Telemetry.start() }   // configure Firebase once, before any UI
        ...
        WindowGroup {
            ContentView()
                ...
                .task {
                    ...
                    TelemetryProfile.refresh(auth: appVM.authViewModel)
