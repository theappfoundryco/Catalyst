...
>
> **Companion docs:** enforced coding ground rules → `Formrules.md` · session status
> + backlog → `HANDOFF.md` · scroll-smoothness deep-dive → `toAvoid.md` · the
> snapshot feature's original design → `CatalystSnapshot-Plan.md` (shipped; see §47) ·
> the Git Graph design → `GitGraph-Plan.md` (shipped; see §48) · deferred/planned
...
| Backend | Two tiers on Cloudflare: read-only static JSON (Pages, `catalyst-3aj.pages.dev`, repo `catalyst_pages`) **+** a dynamic auth/licensing API (Worker `catalyst-api` + D1 + KV, own repo `catalyst_worker`, Git-connected to Workers Builds) — see `goLive.md` |
...
| Source files | ~167 app Swift files (+10 XCTest files) |
| Tests | 10 XCTest files focused on the risky utility/service layer |
...
**import/restore is blue** (see Formrules 4.15). Capture → a green "Snapshot Ready"
...
using the shared sticky footer bar (Formrules 4.14). Restore installs directly
(no dry-run) — per item, exit-code-gated, resumable.

**Safety/consent.** **Secrets are never exported.** The full `~/.zshrc` is
**secret-scrubbed** at capture (`ShellSecretScrubber`, Formrules 2.8): obvious
...
Enforced by `Formrules.md` Part 1:
...
@main CatalystApp
  ...
  → ContentView branches at the ROOT on appVM.isEntitled:
        entitled     → NavigationSplitView (full app: sidebar + toolbar + content)
        not entitled → AuthGateView (plain sign-in window, NO sidebar/toolbar)
...
hacks (see Formrules §6.7). The window now appears already framed and fills content in
...
entitled/sign-in branch), `LaunchScreenView` (**unused since 2026-07-14 — splash removed**),
...
`RemoteCache.swift` (legacy standalone; live cache is in `NetworkConfig.swift`),
...
carefully guarded. **Never run a shell inline** (`Formrules.md` Part 2).
...
(`https://catalyst-3aj.pages.dev/public/...`), authored in the separate
`catalyst_pages` repo. `NetworkConfig` centralizes it.
...
- **Offline / liveness:** `NetworkMonitor` surfaces a colored status
  ...
  (`StatusIndicatorView`). The 30 s ping now hits the **Worker's `/health`**
  (`NetworkConfig.APIEndpoint.healthURL`) — small, purpose-built — after `python_versions.json`
  (the old ping target) was removed.
- **Two-sided contract:** a `Codable` change requires a matching `catalyst_pages`
  change; decode tolerantly (missing keys → empty defaults). No markdown is rendered
  in-app; shell code is never surfaced.

**Auth/subscription API (separate tier, in progress).** Alongside the static Pages JSON,
a dynamic **Cloudflare Worker `catalyst-api`** (own repo `catalyst_worker`, Git-connected to
Cloudflare Workers Builds → push-to-deploy) backs
authentication, entitlement, and Razorpay one-time purchases — the source of truth the app
never bypasses. Bindings: **D1** (`users`/`devices`/`trials`/`subscriptions`) + **KV**
(short-lived OTP + session tokens). **Sign-in is in-app email OTP** (chosen 2026-07 — the
earlier device-code/magic-link flow confused the desktop UX): `POST /auth/email/start`
emails a **6-digit code** → the user types it in the app → `POST /auth/email/verify`
`{email, code, device_id}` returns a **refresh token** (bound to the Mac's `IOPlatformUUID`).
The app then `GET /entitlement` → a short-lived **EdDSA JWT** (plan/exp, computed from server
time) it verifies offline with an embedded public key. Trial + entitlement are
server-authoritative (never the device clock or Keychain). The old device-code/magic-link
endpoints remain but the app no longer uses them. CORS + `OPTIONS` are enabled for the
website. Endpoints + tracker: **`goLive.md`** / `catalyst_worker/README.md`.
Deployed at `catalyst-api.shivanggulati817.workers.dev`.

**App side (shipped, pbxproj `CM`/`CN`).** `Services/AuthService.swift` (email-OTP sign-in,
macOS login Keychain (not Data-Protection — that needs a signing entitlement the unsigned
build lacks; §46), **Ed25519 JWT verify via CryptoKit**, `IOPlatformUUID`, offline
grace) + `ViewModels/AuthViewModel.swift` (email → code state machine) +
`Views/AuthGateView.swift` (two-step email/code gate; plain sign-in **window**, no sidebar).
`ContentView` **branches at the root** on `AppViewModel.isEntitled` (mirrors
`authViewModel.state.isEntitled` via Combine) — entitled shows the full `NavigationSplitView`
app, not-entitled shows only `AuthGateView`; **the whole app is locked until `.entitled`**,
and **signing in auto-starts the 5-day trial**. (2026-07-14: replaced the old overlay-gate +
`WindowChromeFix` + custom `TrafficLights` approach — the sign-in screen no longer covers the
native titlebar, so the real traffic lights are used directly; see Formrules §6.7. Also
2026-07-14: the gate's sign-in card is an **`.overlay` on a flexible background `Color`, not a
`ZStack` sibling** — as a sibling its ~1015pt height became the window's minimum, exceeded the
875pt screen, and made macOS disable native full-screen — green button showed zoom "+" and
vertical resize was locked. Overlay = the background drives window size, card never sets the
floor, SwiftUI re-enables full-screen natively. Full writeup in goLive's 2026-07-14 changelog.)
`Views/UserProfileView.swift` adds the sidebar user row + the **UserView**
sheet (name, plan/renewal, and a 100-avatar picker — `bottts-neutral` as **full-bleed PNGs**
in `Assets.xcassets/Avatars/`; the PDF path was dropped, cairosvg's `svg2pdf` left a margin).
**Website** (`theappfoundryco`, Astro) is **The App Foundry** studio hub at **`theappfoundry.co`**
(Catalyst product page at `/catalyst`) — marketing
+ download only; **there is no web sign-in** (sign-in is entirely in the app). The legacy
`/activate` + `/auth/verify` routes just redirect home until a billing/account page ships
with Razorpay.

**P14 perpetual licence (Razorpay one-time Payment Link).** *Replaced P4 subscriptions entirely
in v1.13 — see the note below.* All on the Worker (no website payment). `POST /license/create`
(or `/license/student/create`) creates a one-time Payment Link for the configured amount, bound
via `notes.user_id` + `notes.kind`, and returns its hosted `short_url`; the app opens it, then
**auto-detects** the flip by polling `/entitlement` (3 s, ~2 min) — plus a **Restore purchases**
action. `POST /webhook/razorpay` (HMAC-verified) handles `payment_link.paid` → writes a
**perpetual grant** (`grants`, far-future sentinel expiry, id `TAFCL…`/`TAFCS…`; ids issued before 2026-07-20 are `TAPC…` and remain valid); `/entitlement`
returns `plan:pro` with `billing_interval: "perpetual"` (or `"student"`). If that webhook is ever
missed, `/entitlement` self-heals by reading the pending link from KV (`stupl:<user_id>`). There
is no cancel — nothing recurs. The in-app **`PaywallView`** (in `UserProfileView.swift`, no new
pbxproj file — one price → "Buy Catalyst Pro") is reachable from the profile sheet **and the
locked gate** (`AuthGateView`) so expired-trial users can upgrade in place.

**Pricing is server-driven**: `PRICE_STANDARD_INR` / `PRICE_STUDENT_INR` in `wrangler.toml` are
both the charged amount AND what `/entitlement` reports to the app, so a price change is a
`wrangler deploy`. INR only — USD awaits Razorpay international approval; an unconfigured
currency returns `price_not_configured` rather than failing at checkout.

> **Subscriptions are GONE (v1.13).** `subscribeCreate`/`subscribeCancel`, `reconcileSubscription`,
> `planInterval`, `userIdForSub`, the entitlement subscription read path and the webhook's
> `subscription.*` branch are all **deleted** — no account was ever billed on one.
> `/subscribe/{create,cancel}` answer **410** so older clients get a clear signal;
> `/subscribe/student/create` still forwards to the student licence. The `subscriptions` D1 table
> remains but is unread. Anything below describing renewals, `cancel_at_cycle_end`, "Manage
> subscription" or `billing_interval: monthly|yearly` is **historical**.

Status (2026-07): **P1–P8, P10 done; P4 backend + in-app paywall done (test mode); P9 Sparkle
auto-update + distribution DONE & verified live** (public `imsg8/Catalyst_Releases`, version-only,
custom badge UI — see §49.7 + `RELEASING.md`); **P12 student pricing done; P11 gift codes planned**
— full tracker in `goLive.md`.

**Backend surface — routing, trust & protection.** Three independent backends; the app talks to
two, the Worker fans out to the rest. Nothing about a paid state is ever decided on the client.

| Edge | Who calls it | What | Protection |
|---|---|---|---|
| **Worker** `catalyst-api…workers.dev` | **app** | `/auth/email/{start,verify}`, `/entitlement`, `/trial/start`, `/license/{create,student/create}`, `/webhook/razorpay`, `/appcast.xml`, `/health` | authed routes require a **Bearer refresh token** (KV-resolved); `/webhook/razorpay` **HMAC-verified**; `/entitlement` returns an **EdDSA-signed** JWT (app verifies offline). CORS `*` is safe — Bearer, no cookies |
| **Pages** `catalyst-3aj.pages.dev` | **app** | read-only static JSON (shortcuts/brew/popular/about), cached via `RemoteCache` | **public by design** — no secrets, no writes; nothing to protect |
| **PyPI** `pypi.org` | **app** | pip's latest version | public |
| **Vercel** `/api/catalyst/send-otp` | **Worker** | sends the OTP email (Nodemailer + Gmail) | **`INTERNAL_EMAIL_SECRET`** header (shared Worker↔Vercel); `GMAIL_*` are Vercel server-only env |
| **Razorpay API** | **Worker** | create one-time payment link | HTTP Basic (`RAZORPAY_KEY_ID/SECRET` — Worker secrets) |
| **`imsg8/Catalyst_Releases`** (public repo + GitHub Releases) | **Worker** | `/appcast.xml` proxies the repo's raw `appcast.xml` | public (feed + binaries); binaries are notarized + EdDSA-signed. Private code repo stays private |

The **website** (`theappfoundry.co`, Astro) is **marketing + download + the `/api/catalyst/send-otp`
function only** — **no app-facing routes, no web sign-in**. The app never hits the website; the
Worker does (for OTP send). Secrets live as **Worker secrets** (`JWT_PRIVATE_KEY`, `RAZORPAY_*`,
`INTERNAL_EMAIL_SECRET`, `RAZORPAY_WEBHOOK_SECRET`) and **Vercel env** (`GMAIL_*`,
`INTERNAL_EMAIL_SECRET`); the app embeds only the Ed25519 **public** key. The refresh token in the
login Keychain is not a source of truth (user-wipeable).

**Known gap — rate limiting (fix before public launch).** The OTP endpoints have **no rate limit**
today: `verify` caps at 5 tries per code (good), but **`start` can be spammed** — email-bombing a
victim and burning the free-Gmail daily quota. Fix with **KV counters** keyed on `rl:email:<addr>`
and `rl:ip:<CF-Connecting-IP>` (e.g. ≤5 `start`/email/hour, ≤20/IP/hour → `429`, generic message),
and apply the same to `/subscribe/create` and `/redeem` (P11). Cloudflare **Rate Limiting Rules**
(dashboard) can front the Worker route as a second layer. Everything else (Bearer-authed, HMAC,
public-read) is already appropriately protected.

**Email delivery (OTP codes) — LIVE.** Real codes deliver to the inbox (verified 2026-07,
from `usecatalystapp@gmail.com`, inbox not spam). Cloudflare Workers **cannot open SMTP** (no
raw TCP), so the code is never emailed *from* the Worker. Two-part design that keeps the **app
and website untouched by any provider swap** (both only ever call the Worker's stable
`/auth/email/*` contract):
- **Now (free stopgap, live):** `emailStart` generates + KV-stores the OTP, then `fetch()`es
  (via `sendOtpEmail()`, guarded by an `x-internal-secret` header) a tiny **Vercel serverless
  function** (`theappfoundryco/api/catalyst/send-otp.js`, Node runtime → SMTP allowed) that sends via
  **Gmail SMTP + Nodemailer** using a Google **app password**. Secrets: `GMAIL_USER`,
  `GMAIL_APP_PASSWORD` live on **Vercel**; a shared `INTERNAL_EMAIL_SECRET` sits on **both**
  Vercel and the Worker so the send route isn't open to the world. The Worker reads
  `EMAIL_ENDPOINT` (a `wrangler.toml` var). In **production** a send failure returns
  `502 email_send_failed`; in **dev** send is best-effort and `dev_code` is still echoed. Caps
  ≈ 500/day (free Gmail), from-address stays `@gmail.com` until a domain + Workspace exist.
  **Deploy gotcha:** Workers Builds did not auto-deploy the GitHub push — the new `emailStart`
  + `EMAIL_ENDPOINT` had to ship via a manual `npx wrangler deploy` (the Git→Workers-Builds
  pipeline still needs fixing). **Only toggle left:** flip Worker `ENVIRONMENT=production` to
  stop the `dev_code` echo (keep local dev echoing via `.dev.vars`).
- **Amazon SES — WIRED (2026-07-17b).** `send-otp.js` picks its provider via `getMailer()`
  (returns `{provider, transporter, from}`), controlled by a **`EMAIL_PROVIDER` Vercel env var
  — the switch: `ses` | `gmail` | `auto` (default = SES if its creds are set, else Gmail).** Flip
  it in the Vercel dashboard + redeploy; no app or website change, no re-release. Region **`eu-north-1`
  (Stockholm)**; from `no-reply@theappfoundry.co` (`SES_FROM`); host
  `email-smtp.eu-north-1.amazonaws.com:587` (STARTTLS; `secure:false`+`requireTLS`, with connect timeouts so a stalled SMTP fails fast instead of an opaque platform 502). `theappfoundry.co` is fully authenticated:
  **DKIM ✓, custom MAIL FROM `mail.theappfoundry.co` ✓ (SPF alignment), DMARC ✓** — Gmail
  "Show original" shows SPF/DKIM/DMARC = **PASS** and it inboxes. Env vars are documented in
  `.env.example`. **Still open:** set the SES creds on **Vercel** + redeploy (they were briefly
  committed to `.env.example` → **rotate**); **AWS production access is pending** (sandbox =
  verified recipients only, 200/day) — lifts server-side, no code change. SES is per-region:
  keep the console, SMTP creds, and sender all on `eu-north-1`.

**App auto-updates (Sparkle) — LIVE.** The Worker serves the feed at **`GET /appcast.xml`**,
proxying the cumulative `appcast.xml` in the PUBLIC releases repo **`imsg8/Catalyst_Releases`**
(`raw.githubusercontent.com/imsg8/Catalyst_Releases/main/appcast.xml`). Full runbook + scripts in
**`RELEASING.md`**; app-side UX in §49.7. **Two independent signatures, one source of truth, one
scripted release:**
...
- **Sparkle EdDSA signature** — lets Sparkle trust the feed. **Public key** in `Info.plist`
  (`SUPublicEDKey`); **private key** in the login Keychain, never in the app/repo. Same
  public-key-in-client / private-key-secret model as the entitlement JWT.
- **Source of truth:** the PUBLIC repo `imsg8/Catalyst_Releases` — every build committed under
  `Versions/<version>/` (zip + `notes.html` + cached-signature `meta.env`) and also uploaded to a
  `vX.Y.Z` GitHub Release (the download host). The private code repo stays private. `SUFeedURL` →
  Worker `/appcast.xml` → raw appcast.
- **Release flow:** `./Scripts/cut_release.sh` — preflight (asserts Release config, hardened
  ...
  `sign_update` → `Scripts/make_appcast.py` regenerates the cumulative `appcast.xml` → `gh release
  create` → commit + push `Catalyst_Releases`. **Version-only:** bump `MARKETING_VERSION` only.
...
(Formrules 2.10). `AuthService` logs nothing; `AsyncProcessRunner` logs no command strings;
...
match the grammar exactly (`Formrules.md` Parts 3–4). This section doubles as the
...
- **Inputs:** `CompactInputField` / `SearchBarView` — one field style app-wide.
  ...
  both the field and its padded container need `.onTapGesture` (Formrules 12.37).
...
- **Sticky action footer:** a selection/commit screen pins its primary action in a
  ...
  grammar via the shared `SnapshotFooterBar` (Formrules 4.14).
- **Per-flow accent color:** a directional flow keeps ONE accent throughout —
  **green** for capture/export/backup, **blue** for import/restore — so its landing
  ...
  shell/git purple, python/pip/projects blue). (Formrules 4.15.)
...
GitGraphView); the **auth/entitlement** files use the `CM` prefix (`CM…B` AuthService ·
`C` AuthViewModel · `D` AuthGateView); the **UserView/avatars** file uses the `CN` prefix
(`CN…D` UserProfileView). (100 avatar imagesets in `Assets.xcassets/Avatars/` need **no**
...
## 43. Testing
`CatalystTests/` (XCTest) targets the **risk-bearing utility/service layer**, not UI:
`InputSanitizerTests`, `PrivilegesServiceTests`, `AsyncProcessRunnerTests`,
`BrewPathManagerTests`, `VersionComparatorTests`, `NetworkConfigTests`, `ModelTests`,
`DestructiveTests` (deletion safety), plus broader `AdditionalTests` / `CatalystTests`.
The emphasis on sanitization, privilege, and destructive-path tests reflects that the
highest-stakes code is shell execution and file deletion — add tests there when you
touch those paths.
...
| Restyle the sticky action footer | `SnapshotFooterBar` in `Views/SnapshotView` / the bar in `CruftSweeperCards` (Formrules 4.14) |
...
- **Filesystem scanners identify by MARKER, not name** (a `node_modules`/`venv`/
  `.gradle` needs a sibling project marker; see `Formrules.md` 8.3).
- **Brew paths are `async`** — access only from async contexts.
- **The backend is a two-sided contract** — a `Codable` change needs a matching
  `catalyst_pages` change.
...
is always `.skipped`, never `.failed`. See Formrules 12.38.
...
Landing → Working → Capture-ready → Restore-plan. Two **color-coded flows** (Formrules
...
shared `SnapshotFooterBar` (Formrules 4.14) — a single blue **Restore N** button that
...
`GitGraph-Plan.md`. Deferred items live in `aheadFeatures.md`.
...
the source as it evolves. Deferred work lives in `aheadFeatures.md`.*

---

# §49 — Monetization, Telemetry & Distribution (2026-07 additions)

Extends §36 (networking / auth / entitlement). The app still trusts **only** the signed EdDSA entitlement JWT; everything here is server-authoritative.

## 49.0 — 2026-07-16 updates (read alongside the sub-sections below)
- **[HISTORICAL — subscriptions removed v1.13]** Entitlement honors the PAID period. `/entitlement` grants `pro` for `status ∈ {active, cancelled, completed} && current_period_end > now` (was `active`-only). So cancelling in-app (`cancel_at_cycle_end=1`, stays `active`) and revoking the UPI Autopay mandate (Razorpay `subscription.cancelled` → D1 `cancelled`) both keep Pro until the paid period ends, with `willCancel=true` ("Pro · ends <date>"). `past_due` (failed renewal) is excluded.
- **`subscription_id` / `grant_id`** in `/entitlement` → shown in the profile as support handles (`AuthViewModel.subscriptionId`/`grantId` → `UserProfileView`), each with a **copy button** (green-tick 2 s, `CopyButton` in `UserProfileView.swift`). **Updated 2026-07-17b:** `subscription_id` now surfaces for **any subscription that grants Pro, including cancelled-but-still-in-period** (was `active`-only — a cancelled-till-end user wrongly saw no ID). Added **`grant_id`** as the student/gift parallel. Both gated on the **same `> now` entitlement guard** (`subHonored` inside the `status ∈ {active,cancelled,completed} && current_period_end > now` branch; `grant_id` from `WHERE expires_at > now`), so no ID shows past its date and there's no parallel date check to drift. Payment ID intentionally not shown.
- **Live entitlement re-check (client):** `AuthViewModel.startEntitlementMonitor()` (**4h** while entitled — was 60s, which alone capped the product at ~200 DAU on Cloudflare's free tier; P14) + `ContentView` `scenePhase == .active` re-fetch entitlement, bounded ≤8s, offline-safe. `AuthError.deviceReleased` (distinct 401) → graceful local sign-out. Catches multi-device eviction + renewal/lapse without a relaunch (closes the old "only checked at cold launch" gap).
- **Offline clock-rollback guard:** `AuthService` keeps a monotonic server-clock floor (max JWT `iat`, Keychain); `cachedEntitlement()` rejects a cached token if local time is rolled back past it (10-min skew).
- **Detection gating (perf, not entitlement):** first `fullRefresh()` runs once when auth becomes `.entitled` (not at raw launch) + a global `AsyncConcurrencyLimiter(6)` on shell spawns — fixes the first-login hang / Git Graph spinner. See Formrules 12.12.
- **Razorpay operational gotchas** (webhook mandatory, key/plan mode match, immutable plans, period↔slot, links≠subs, dashboard subs unattributed, prices hardcoded): consolidated in Formrules 12.15 and the goLive 2026-07-16 changelog.
- **Sparkle:** `automaticallyDownloadsUpdates`/`automaticallyChecksForUpdates` set explicitly in `UpdaterController.init` (Info.plist default is skipped when `SUEnableAutomaticChecks` is set) — the fix for the badge that never downloaded. See §49.7, Formrules 12.11.

## 49.1 Entitlement response — fields the app reads
`GET /entitlement` now returns, alongside `entitlement_token` (the signed JWT) and `plan`:
- `will_cancel` — Pro is scheduled to end (cancelled but still in the paid period). Also a `cancel` JWT claim so offline reads know. UI: badge "Pro · ends <date>", row "Access until".
- `billing_interval` — **now `"perpetual" | "student" | null`**; the monthly/yearly values below are historical. Originally `"monthly" | "yearly" | "student" | null` (derived by `planInterval()` from the Razorpay `plan_id` the webhook stored, or the `monthly_INR`/`yearly_INR` recorded on create). UI: "Pro · Monthly/Yearly/Student".
- `email` — the account's primary email (from `users`), so the app shows it regardless of when the user signed in.
- `student_eligible` — academic primary email OR a verified secondary academic email; drives the paywall student option.
- `student_email` — the verified academic email attached to the account (shown in the profile sheet).
- `subscription_id` — **always `null` since v1.13** (nothing can create a subscription). Historically: the Razorpay subscription id whenever a subscription was what granted Pro, **including a cancelled-but-still-within-paid-period sub** (guarded by `subHonored`, set inside the same `status ∈ {active,cancelled,completed} && current_period_end > now` branch that grants Pro; 2026-07-17b). `null` for trials, lapsed subs, and pure student grants. Shown as a "Subscription ID" row (copy button).
- `grant_id` — the **licence id** (`TAFCL…`/`TAFCS…`; pre-2026-07-20 ids are `TAPC…`) whenever a live grant is what grants Pro; shown as the **"Licence ID"** row (was "Grant ID"). Originally described as a one-time student/gift grant id (worker query filters `WHERE expires_at > now`, so it self-clears at expiry). `null` otherwise. Shown as a "Grant ID" row (copy button) — the student parallel to `subscription_id`.

`Entitlement` (Swift, `AuthService.swift`) carries `willCancel`, `interval`, `email`, `studentEligible`, `studentEmail`, `subscriptionId`, `grantId`, `formerSubscriber`. `subscriptionId`/`grantId` are **nil on the offline JWT path** (they come only from the live `/entitlement` response, like the other server-only fields).

## 49.2 Self-healing reconciliation
`/entitlement` calls `reconcileSubscription()` — a live `GET /v1/subscriptions/{id}` to Razorpay — and corrects the D1 row if it drifted from what webhooks said (stuck `created` after a paid checkout, or `active` after a cancel). Maps active→active, halted→past_due, cancelled/completed/expired→cancelled; leaves created/authenticated/pending. **Fails open** (a Razorpay hiccup never downgrades a valid user). Student payment links self-heal the same way via the `stupl:<uid>` KV marker. Net effect: activation + cancellation are correct even if a webhook is missed — but keep all 5 `subscription.*` events **and** `payment_link.paid` enabled on the Razorpay webhook.

## 49.3 Cancel / resubscribe model
The app only ever does `cancel_at_cycle_end` (keeps Pro to the period end, no auto-renew). Razorpay keeps such a sub **active** with a next-billing date until cycle end — that is correct, not a bug. Resubscribe is **lapse-only**: while cancelled-but-active there's no repurchase (nothing to buy); once it lapses, the locked gate offers a fresh subscription. `subscribeCreate` refuses a 2nd sub while one is active (no duplicates). The old scheduled-successor design (`start_at` + `pending_subscription_id`) was removed.

## 49.4 Rate limiting & abuse
KV fixed-window `rateLimited(bucket, limit, windowSec)` (keys `rl:*` in `SESSIONS`). Applied to `/auth/email/start` (IP 20/h, email 4/h, device 6/day), `/auth/email/verify` (IP 30/h), magic-link, device/start. User rows are created only on successful verify. Trial abuse capped by `devices.trialed` (one machine = one trial); **concurrent-use** capped by single-seat device binding (§49.9).
**Flushing a lockout — gotcha:** the app's "Too many attempts" is the `rate_limited` 429 (NOT `too_many_attempts`, which is the verify-side `otp:<email>` tries≥5 counter). It can come from three `/auth/email/start` buckets: `rl:otp:email:<email>` (4/h), `rl:otp:ip:<ip>` (20/h), and **`rl:otp:dev:<device>` (6/day — the usual culprit when testing repeatedly on one Mac)**. The IP and device buckets are **not** keyed by email, so deleting `rl:otp:email:<email>` alone won't clear them. Use `Scripts/flush_sessions.sh`, which **purges the whole `rl:` prefix** (plus pending `otp:`/`sotp:`) — it leaves `apprefresh:*` refresh tokens intact, so signed-in users aren't evicted.

## 49.5 P12 — Student discount (grants model)
Non-recurring: a one-time Razorpay **Payment Link** (₹1,499) → on `payment_link.paid`, insert a 1-year row in the new `grants` table (`source='student'`). Entitlement = `max(active sub, unexpired grant)`. Eligibility: `isAcademicEmail` on the primary email, OR a **secondary academic email verified by OTP** (`/student/verify/start` + `/confirm` → `users.student_email` + `student_verified_at`, valid ~1yr). **Invariant: one academic email backs the discount on exactly one account, ever** — enforced by a UNIQUE index on `student_email` plus cross-checks in verify (vs other accounts' primary/student emails) and in sign-in (`email_reserved_student` blocks logging in with an email that's another account's student email). The `grants` table also backs P11 gift codes.

## 49.6 Telemetry (Firebase) — the facade
`Telemetry/` (4 files). **Only `Telemetry.swift` imports Firebase** (`FirebaseAnalytics` + `FirebaseCrashlytics`). `AppEvent` (event catalog, no vendor types), `AppUserProperty` (segmentation), `TelemetryProfile` (derives props from app state). Call sites are one line: `Telemetry.log(.featureOpened(feature:))`, `Telemetry.setUser(id:tier:)`, `Telemetry.nonFatal(_:context:)`. Wired at 3 central points only (no per-view edits): `CatalystApp` (`start()` + `app_open` + profile), `AppViewModel.currentScreen` didSet (`feature_opened`), `AuthViewModel` (auth/subscription/student events + `setUser` keyed by the device UUID — never the email). Dev builds print; Release sends. To swap providers or add consent/opt-out, edit only `Telemetry.swift`.

## 49.7 Sparkle auto-update (app side)
`UpdaterController` (in `CatalystApp.swift` — avoids a new pbxproj entry) is now an `ObservableObject` wrapping `SPUStandardUpdaterController` and acting as both `SPUUpdaterDelegate` and `SPUStandardUserDriverDelegate` (all method signatures verified against the Sparkle 2.x headers). `startingUpdater: true` schedules Sparkle's **own** hourly background checks. **Check-on-open (2026-07-14c):** the root `.task` calls `UpdaterController.shared.checkOnLaunch()`, which forces one background check ~3s after launch *guarded by `updater.canCheckForUpdates`* (false while a Sparkle session is already running → no `sessionInProgress` collision). This was added because relying on Sparkle's scheduler alone meant "open the app" frequently checked **nothing** — Sparkle only launch-checks once `SUScheduledCheckInterval` (1h) has elapsed since the last check and defers the first check after a fresh install, so the badge often didn't appear on open (diagnosed via `log stream` showing zero Sparkle activity on launch, feed confirmed correct server-side). `checkInBackground()` remains as a manual trigger. On find, `didFindValidUpdate` lights the badge immediately; auto-download then drives it to "Relaunch to update". `Info.plist`: `SUFeedURL` → Worker `/appcast.xml`, `SUPublicEDKey`, `SUEnableAutomaticChecks`, `SUAutomaticallyUpdate` (silent background download), `SUScheduledCheckInterval = 3600` (hourly).

**Custom gentle UX (Claude-app style).** Instead of Sparkle's own windows, updates download silently and surface a sidebar badge — `SidebarUpdateBadge` (live wrapper) → `UpdateBadgeView` (pure visual, driven by an explicit `UpdatePhase`: `.available` / `.downloading` / `.readyToRelaunch`; both in `ContentView.swift`, `#Preview`-covered). The badge is a plain icon + one line of text (no version line, no animation); tapping the ⓘ opens `ReleaseNotesPopover` (renders the appcast `<description>` HTML → `AttributedString`). Key hook: `updater(_:willInstallUpdateOnQuit:immediateInstallationBlock:)` fires the instant the silent download completes — we stash the block, show **"Relaunch to update"**, and return `true`; tapping the badge calls the block → installs + relaunches with **no Sparkle window** (falls back to `checkForUpdates()` if unavailable). `supportsGentleScheduledUpdateReminders` + `standardUserDriverShouldHandleShowingScheduledUpdate → false` suppress Sparkle's own popup. **Gotcha:** the visible reminder is deferred in auto-download mode (Sparkle's `SUScheduledImpatientCheckInterval`), so the badge is driven off the lifecycle callbacks above, not the gentle-reminder show callback.

**Distribution — see RELEASING.md.** Binaries + appcast live in the PUBLIC repo **`imsg8/Catalyst_Releases`** (`Versions/<version>/` archive + `appcast.xml`); the private code repo stays private. **Version-only:** bump `MARKETING_VERSION` only — `CURRENT_PROJECT_VERSION = $(MARKETING_VERSION)` makes `CFBundleVersion` track it, and `sparkle:version` = the marketing version (Sparkle's comparator orders 1.1 > 1.0). Signing note: the target must NOT pin `CODE_SIGN_IDENTITY[sdk=macosx*] = "-"` (ad-hoc) — it made the app ad-hoc while Sparkle.framework was team-signed and Hardened Runtime aborted with "different Team IDs". Use Apple Development + the team.

## 49.8 Website (Razorpay activation surface)
`catalyst_website` gained `/pricing`, `/terms`, `/privacy`, `/refunds`, `/contact` (footer-linked) + `LEGAL`/`PRICING` in `consts.ts`. These exist to satisfy Razorpay's website review; payment itself is still Razorpay hosted checkout from the app. The `LEGAL` block has TODO placeholders (legal name/phone/address) that must be filled before submitting the site to Razorpay.

## 49.9 Single-seat licensing — one account = one active Mac (2026-07-14b) — LIVE
**The "duplicate-login guard."** Stops one account from running on unlimited Macs (the "one license, 100 people" vector). **Status: LIVE** — Worker deployed + `migrations/0001_device_binding.sql` applied to remote D1 on 2026-07-14; shipped in app **v1.4**. The seat is stored **on the `users` row**, not as a loose pile of `devices` rows: `active_device_id` (the one bound Mac's `IOPlatformUUID`), `active_refresh_id` (its live refresh token, so it can be revoked), `active_device_since`, `delink_count`, `delink_window_start`. `IOPlatformUUID` is stable across reinstall / logout-login, so the same Mac never costs a seat.

**D1 schema-change workflow (this was the template).** `schema.sql` is the fresh-install shape; **existing** live DBs are altered by a numbered file under `catalyst_worker/migrations/` applied with `npx wrangler d1 execute catalyst-db --remote --file=migrations/NNNN_*.sql` (also `--local` for the dev DB — a `duplicate column` error there just means it was already applied). **Apply the migration BEFORE/at the deploy that needs it:** deploying code that reads new columns against an un-migrated DB throws `D1_ERROR: no such column` → a 500 that surfaces in-app as the generic *"invalid or expired"* on a correct OTP (this bit us once; see the goLive 2026-07-14b deploy gotcha).

**Bind + block (login).** `/auth/email/verify`: NULL seat → bind this device; same device → allow (revokes the prior token for that device first); **different device → `409 device_limit`** returning a short-lived `release_token` (KV `release:<tok>`, minted only after a valid OTP) + `delink_remaining`.

**Move the seat (delink).** `POST /auth/device/release` takes the `release_token`, spends one delink (capped **`DELINK_CAP = 2` per rolling `DELINK_WINDOW = 90d`**, auto-resets via `delinksLeft()`), **revokes the old Mac's refresh token**, then binds + mints for the new Mac. Over the cap → `429 delink_cap_reached` (no self-serve path beyond that yet — manual/gift is a TODO).

**Evict (entitlement gate).** `/entitlement` rejects any token whose `device_id ≠ users.active_device_id` with **`401 device_released`** (and deletes the stale token) — this is what actually signs the released Mac out, on its next check, bounded by the ~3-day JWT grace. Legacy/unbound accounts **lazy-bind** to the first caller; any other still-live device collapses to `device_released`.

**Sign-out ≠ delink.** `POST /auth/signout` revokes only *this* Mac's token but **keeps the seat bound** (clears `active_refresh_id` only if it named this token). This closes the old token-revocation TODO and is what stops serial "take turns" sharing — only the capped release frees a seat.

**App side.** `AuthService`: `AuthError.deviceLimited(releaseToken:remaining:)` (parsed in `request()` from the 409 body), `releaseDevice()`, `signOutServer()`. `AuthViewModel`: new `.deviceLimited(email:releaseToken:remaining:)` state, `submitCode` branch, `releaseThisDevice()`, `signOut()` now calls the server first. `AuthGateView`: a `deviceLimited` card ("Sign in here & sign out the other Mac" + releases-remaining). The evicted Mac's next `/entitlement` 401 → the existing `.unauthorized` path signs it out.

## 49.10 Legal consent — versioned Privacy/Terms re-consent (2026-07-17)
**What/why.** Legal docs change; when they do, every user must re-accept. A **blocking, non-dismissable** sheet gates the entitled app until accepted; acceptance is stored **per-Mac** and survives force-quit/relaunch; a **14-day** check catches new versions.

**Files.** `Catalyst/LegalConsent.swift` (in the synchronized `Catalyst/` group → auto-registers, no pbxproj entry) holds `LegalConfig`, `LegalVersions`, `LegalConsentRequirement`, `LegalConsentViewModel` (`@MainActor`), and `LegalConsentSheet`. Storage fields live in `ConfigStore` (`acceptedPrivacyVersion`, `acceptedTermsVersion`, `cachedPrivacyVersion`, `cachedTermsVersion`, `lastLegalCheckISO`, `legalAcceptedAtISO`) + `recordLegalAcceptance` / `recordLegalRemote` accessors. `AppViewModel` owns `legalViewModel` and mirrors `$requirement → legalRequirement` (same pattern as `isEntitled`). `ContentView` presents the sheet; `AuthGateView` records consent from the sign-in checkbox.
...
**Flow.** New sign-in → the required `AuthGateView` checkbox calls `recordSignInConsent()` (records current versions) so the sheet won't immediately re-prompt. Everyone else — existing installs (no stored acceptance), new-Mac logins, later version bumps — is caught by the blocking sheet. Copy adapts: "We've updated…" vs "Please review…", and only the doc(s) that changed. Force-quit mid-sheet → recomputed from persisted state on next launch, so it re-appears.
...
## 49.13 Session changes — auth gates, automation, shortcuts, hardening (2026-07-17 → 07-18)

Cross-references Formrules 12.27–12.36. Highlights so a reviewer knows the current guarantees:

**Entitlement gates.** (a) *One free trial per Mac* — the trial is device-bound (`devices.trialed`); `/trial/start` returns `403 device_already_trialed`, now surfaced (not `try?`-swallowed) and persisted per-Mac (`UserDefaults "trial.deviceAlreadyUsed"`) → a `.deviceTrialed` `LockKind` shows the friendly "One free trial per Mac" gate instead of a false "your trial ended." (b) *former_subscriber* is now paid-states-only (`active/cancelled/completed/past_due/halted`) — an abandoned `created` checkout no longer greets a new account with "Welcome back." Precedence: former-subscriber → device-trialed → trial-ended. (c) Paywall Back is stripped once `checkoutStarted`, with a cached-URL "Reopen checkout" so a payment can't be abandoned/double-started. (Formrules 12.29, 12.30, 12.35)

**Automation / privileged exec.** Sending Apple events to Terminal (`TerminalService`) needs BOTH `com.apple.security.automation.apple-events` (entitlement) and `NSAppleEventsUsageDescription` (Info.plist) under Hardened Runtime — else -1743 and silent no-op; the handler now opens the Automation settings pane. `PrivilegesService.runSudo` pins `LC_ALL=C` so the stale-password re-prompt survives localized Macs. `AsyncProcessRunner` guards every `terminate()` with `isRunning` (an unlaunched-task terminate crashed the app on a cancel race). (Formrules 12.28, 12.32, 12.33)

**SmartShortcuts.** Data reads from deployed CF Pages (`catalyst-3aj.pages.dev`) with two client caches (7-day `RemoteCache` disk + 14-day UserDefaults snapshot); `refresh()`/`forceReload()` now bust both via `clearShortcutsCaches()` (targeted `RemoteCache.clear(url)` leaves the brew/pypi catalogs) — a removed shortcut no longer lingers. Catalog trimmed: 11 duplicates of native features removed, 7 new shortcuts added. Detail view redesigned (hero card + one consolidated content card, name-aware usage/examples via `applyingName`, canonical `CompactInputField`, install log removed, copy button beside the command). `about.json` is bundled in-app and needs a `versions[<MARKETING_VERSION>]` block each bump. (Formrules 12.31, 12.36)

**Input validation.** PARTIAL — only the venv name field (`VirtualEnvCreationSheet`/VM: `^\.?[A-Za-z0-9][A-Za-z0-9_-]*$`, ≤64, inline message, Create gated). The single `Validators` utility + all other fields remain deferred (goLive §9, `aheadFeatures.md`). (Formrules 12.27)

**Grant IDs** are branded `TAFC<hex>` — renamed from `TAPC` on 2026-07-20 for NEW ids only; anything matching on a licence id must accept both prefixes indefinitely. Display handle only. **Non-GST invoices shipped in v1.18** (`invoices` table, `TAFC-INV<YYYY>-<6>`, rendered on device) — no longer deferred. (Formrules 12.34, 12.43–12.45)
