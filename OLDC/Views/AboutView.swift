...
/// `version` + the signed-in `email` so the destination (GitHub issue form / Tally form)
/// prefills them. Replaces the deprecated `about.json` link fields.
enum CatalystLink: String, CaseIterable {
    ...
    /// Capture routes (bug/feature/feedback/support) get version+email prefill; the marketing
    ...
    /// The stable URL, with `version` (+ optional `email`) appended for capture routes. The
    /// Vercel middleware forwards these onto the real destination for prefill.
    func url(version: String, email: String?) -> URL? {
        var comps = URLComponents(string: Self.base + rawValue)
        if prefills {
            var items = [URLQueryItem(name: "version", value: version)]
        ...
        ScrollView {
            VStack(spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    // Config). Version + signed-in email are appended so bug/feature/feedback
                    // land prefilled. `auth.email` is read from UserDefaults (persisted by
                    // AuthViewModel) so About needs no direct auth dependency.
                    LazyVGrid(
